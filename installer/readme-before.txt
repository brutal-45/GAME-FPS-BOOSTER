╔══════════════════════════════════════════════════════════════════╗
║                                                                  ║
║     ██████╗ ██╗   ██╗██████╗ ██╗   ██╗████████╗███████╗         ║
║     ██╔══██╗██║   ██║██╔══██╗██║   ██║╚══██╔══╝██╔════╝         ║
║     ██████╔╝██║   ██║██████╔╝██║   ██║   ██║   █████╗           ║
║     ██╔══██╗██║   ██║██╔══██╗██║   ██║   ██║   ██╔══╝           ║
║     ██║  ██║╚██████╔╝██████╔╝╚██████╔╝   ██║   ███████╗         ║
║     ╚═╝  ╚═╝ ╚═════╝ ╚═════╝  ╚═════╝    ╚═╝   ╚══════╝         ║
║                                                                  ║
║              ULTIMATE GAMING PERFORMANCE BOOSTER                 ║
║                       Version 1.0.0                              ║
║                                                                  ║
╚══════════════════════════════════════════════════════════════════╝

                    DEVELOPED UNDER BRUTAL TOOLS

Thank you for choosing BRUTAL-FPS!

WHAT'S INCLUDED:
  • One-click FPS boost functionality
  • Real-time performance monitoring
  • Game profile management
  • Emulator optimization (BlueStacks, LDPlayer, Nox)
  • System tweaks and optimizations
  • Achievement system
  • Multiple themes (Dark, Cyber, Inferno, Arctic, Military)

INSTALLATION:
  1. Click Next to begin installation
  2. Accept the license agreement
  3. Choose installation location
  4. Select additional shortcuts
  5. Click Install

REQUIREMENTS:
  • Windows 7/8/10/11 (64-bit)
  • 4 GB RAM (recommended)
  • 100 MB free disk space

AFTER INSTALLATION:
  • Launch BRUTAL-FPS from desktop or start menu
  • The app will open in your default browser
  • Click "BOOST" to optimize your gaming performance

SUPPORT:
  Website: https://brutal-tools.com
  Discord: https://discord.gg/brutal-fps

LICENSE: MIT License - Free for personal and commercial use

        "UNLEASH THE BEAST IN YOUR PC" 🖥️🔥
